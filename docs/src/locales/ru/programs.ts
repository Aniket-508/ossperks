const programs = {
  ...
  listing: {
    description:
      "Бесплатные инструменты, кредиты и сервисы, доступные для проектов с открытым исходным кодом. Просматривайте по категориям и находите программы, для которых подходит ваш проект.",
    heading: "Программы",
  },
  more: "ещё {count}",
  sections: {
  ...
  submit: {
    ...
    description:
      "Есть программа для предложения? Отправьте её описание, и мы добавим её в список.",
    form: {
      ...
      eligibilityHelp:
        'Добавьте одно требование на поле (напр. "Проекты с открытым кодом", "Публичный репозиторий на GitHub").',
    ...
    validation: {
      ...
      perkRequired:
        "Необходимо хотя бы одно преимущество с заголовком и описанием",
